...
function Drag.new(frame)
	local ok, mouseEnter = pcall(function() return frame.MouseEnter end)
	if not ok then return end

	frame.Active = true
	mouseEnter:connect(function()
		local inputConn, leaveConn
		...
		inputConn = frame.InputBegan:connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				...
				while Heartbeat:wait() and UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
					pcall(function()
						frame:TweenPosition(UDim2.new(0, Mouse.X - offset.X, 0, Mouse.Y - offset.Y), "Out", "Linear", 0.1, true)
					end)
				end
			end
		end)

		leaveConn = frame.MouseLeave:connect(function()
